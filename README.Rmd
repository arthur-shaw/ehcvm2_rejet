...
Pour que le programme s'exécute, il doit savoir où il se trouve dans votre système de fichiers. Pour orienter le programme, indiquez le chemin du fichier à la racine du projet (c'est-à-dire le répertoire dans lequel se trouve `ehcvm2_rejet.Rproj`). Ce chemin doit être spécifié comme la valeur de `proj_dir` (c'est-à-dire entre guillemets)

Ce faisant, suivez les règles répétées dans le morceau de code ci-dessous :

- Utiliser `/` au lieu de `\` dans les chemins. Dans R, `\` veut dire quelque chose de spécial.
- Tâcher de mettre `/` à la fin des chemins. Ceci aidera dans la construction du chemin des fichiers relative à la racine du projet.
